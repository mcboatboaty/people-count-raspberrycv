...
task<void> MainPage::InitializeCameraAsync()
{
		...
		.then([this](DeviceInformation^ camera)
	{
			...
			.then([this]()
		{
			...
			GetPreviewFrameButton_Click(nullptr, nullptr);
			/***************/
...
void MainPage::captureThread() {
	...
	auto previewProperties = static_cast<MediaProperties::VideoEncodingProperties^>(mp->getMediaCapture1()->VideoDeviceController->GetMediaStreamProperties(Capture::MediaStreamType::VideoPreview));
	unsigned int videoFrameWidth = previewProperties->Width;
	unsigned int videoFrameHeight = previewProperties->Height;
	auto videoFrame = ref new VideoFrame(BitmapPixelFormat::Bgra8, videoFrameWidth, videoFrameHeight);
	...
	content->Insert(ref new Platform::String(L"ID"), Pi_Serial);
	content->Insert(ref new Platform::String(L"Label"), Pi_Name);
	IHttpContent^ httpcontent = ref new HttpFormUrlEncodedContent(content);

	//send the request to the web app
	x->PostAsync(uri, httpcontent);

	while (true) {
		mp->getMediaCapture1()->GetPreviewFrameAsync(videoFrame);
		auto previewFrame = videoFrame->SoftwareBitmap;
		INT32 m = 0;
		m += mp->findPeople(previewFrame, pMOG2, mask);
		if (m == 1) {
			/*SEND POST REQUEST*/
			HttpClient^ x = ref new HttpClient();
			...
			Platform::Collections::Map<String^, String^> ^content = ref new Platform::Collections::Map<String^, String^>();
			content->Insert(ref new Platform::String(L"ID"), Pi_Serial);
			wchar_t m_reportFileName[2];
			swprintf_s(m_reportFileName, L"%d", m);
			content->Insert(ref new Platform::String(L"line"), ref new String(m_reportFileName));
			IHttpContent^ httpcontent = ref new HttpFormUrlEncodedContent(content);
...
Platform::Agile<Windows::Media::Capture::MediaCapture^> MainPage::getMediaCapture1() {
...
void MainPage::CaptureLoaded(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
...
int MainPage::findPeople(SoftwareBitmap^ frameToCovert, cv::Ptr<cv::BackgroundSubtractor> &pMOG2, cv::Mat &fgMaskMOG2) {
	int people = 0;

	//Convert SoftwareBitmap ---> cv::Mat
	WriteableBitmap^ wb = ref new WriteableBitmap(frameToCovert->PixelWidth, frameToCovert->PixelHeight);
	frameToCovert->CopyToBuffer(wb->PixelBuffer);
	...
	cv::Mat frame(wb->PixelHeight, wb->PixelWidth, CV_8UC4, extracted);
	cv::cvtColor(frame, frame, CV_RGBA2BGRA);
	...
	/*NEW CODE*/
	cv::Mat img = fgMaskMOG2;
	double threshold = 128; // needs adjustment.
	int n_erode_dilate = 1; // needs adjustment.

	cv::Mat m = img.clone();
	//cv::cvtColor(m, m, CV_GRAY2BGRA); // convert to glayscale image.
	cv::blur(m, m, cv::Size(15, 15));
	//cv::threshold(m, m, threshold, 150, CV_THRESH_BINARY_INV | THRESH_OTSU);
	...
	//find contours and update the m image
	cv::findContours(m, contours, CV_RETR_LIST, CV_CHAIN_APPROX_NONE, cv::Point(0, 0));
	...
	//draw a bounding rectangle for every detected object
	for (int i = 0; i < contours.size(); i++)
	{
		if (boundRect[i].width > 100 && boundRect[i].height>100) {
			drawContours(frame, contours_poly, i, cv::Scalar(100, 100, 200), 1, 8, std::vector<cv::Vec4i>(), 0, cv::Point());
			rectangle(frame, boundRect[i].tl(), boundRect[i].br(), cv::Scalar(100, 100, 200), 5, CV_16SC4);
			/*if ((boundRect[i].x + boundRect[i].width / 2) < 290 && (boundRect[i].x + boundRect[i].width / 2)>210) {
				line(frame, cv::Point(250, 0), cv::Point(250, 500), cv::Scalar(255, 255, 0), 5, 8, 0);
				people = people + 1;
			}*/
			people = people + 1;
		}
	}
	line(frame, cv::Point(250, 0), cv::Point(250, 500), cv::Scalar(100, 100, 100), 5, 8, 0);
	return people;
}
